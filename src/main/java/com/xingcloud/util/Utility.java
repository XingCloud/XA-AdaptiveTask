...
public class Utility {
  ...
  public static Map<Index, Integer> Set2Map(Set<Index> indexSet) {
    Map<Index, Integer> indexMap = new HashMap<Index, Integer>();
    for (Index index : indexSet) {
      indexMap.put(index, index.getFreq());
    }
    return indexMap;
  }

  public static Map<Index, Integer> MergeMap(Map<Index, Integer> src, Map<Index, Integer> dst) {
    for (Index index : src.keySet()) {
      Integer integer = dst.get(index);
      if (integer == null) {
        dst.put(index, index.getFreq());
      } else {
        integer += index.getFreq();
        dst.put(index, integer) ;
      }
    }
    return dst;
  }
